...
// Fondo animado del hero: una red de casitas, corazones y personitas que se
// conectan entre sí, laten como un pulso, y de tanto en tanto se mandan un
// "envío de ayuda" (un punto dorado viajando por la conexión). Así el hero
// anticipa visualmente el propósito del proyecto: una red que llega a los
// hogares que lo necesitan.
//
// Adaptado de la versión de Corazones Unidos: esa tenía un cuarto tipo de
// nodo ("manos con corazón") dibujado con la ligadura del ícono de Material
// "volunteer_activism" -- frontend-v2 no usa Angular Material, así que ese
// tipo de nodo se saca en vez de cargar una fuente de íconos solo para esto.
interface Nodo {
  x: number;
  y: number;
  vx: number;
  vy: number;
  tipo: 'casa' | 'corazon' | 'persona';
  fase: number;
}
...
// Color cálido ya usado en el hero (el "eyebrow" de arriba de todo): las
// personitas lo toman para distinguirse del blanco de las casitas.
const COLOR_PERSONA = '246, 220, 194';
const COLOR_ACENTO = '230, 95, 50'; // var(--brand)
...
export class HeroNetworkComponent implements AfterViewInit, OnDestroy {
  ...
  // Zona (elipse) que ocupa el texto del hero: los nodos evitan nacer ahí y
  ...
  private calcularZonaTexto(): void {
    const canvas = this.canvasRef.nativeElement;
    // El padre inmediato del canvas es el host <app-hero-network>:
    // ".hero-content" es hermano del host, así que hay que subir un nivel
  ...
  // Qué tan "adentro" de la zona de texto está un punto (elipse): <1 es
  // adentro, >=1 es afuera. Usarlo en vez de un rectángulo exacto alcanza
  ...
  private generarNodos(): void {
    ...
    const tipos: Nodo['tipo'][] = ['casa', 'corazon', 'persona'];
  ...
  private actualizar(): void {
    ...
    // Cada tanto, un envío de ayuda "viaja" entre dos nodos conectados.
