...
export class OrganizacionPerfilComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (!id) {
      this.errorMessage.set(
        'Falta indicar qué organización mostrar.',
      );
  ...
  private cargarPerfil(id: string): void {
    ...
    forkJoin({
      ...
      recursos:
        this.http.get<Publicacion[]>(
          `/api/organizations/${id}/resources`,
        ),

      necesidades:
        this.http.get<Publicacion[]>(
          `/api/organizations/${id}/needs`,
        ),
    }).subscribe({
      next: ({
        organizacion,
